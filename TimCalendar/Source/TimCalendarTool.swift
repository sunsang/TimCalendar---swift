...
public class TimCalendarTool: NSObject {
    ...
    // 当月的第一天是星期几
    public func firstDayInFirstWeekThisMonth(date: Date) -> Int {
    ...
    public func StringMonth(date: Date) -> String {
        var str = ""
    ...
    public func dateByday(day: Int, date: Date) -> Date {
    ...
    public func betweenOfDay(minDate: Date, maxDate: Date, date: Date) -> Bool{
    ...
    public func betweenOfMonth(minDate: Date, maxDate: Date, date: Date) -> [Bool]{
        let dateFormatter = DateFormatter.init()
...
public class TimCalendarConfig: NSObject{
    ...
    public static let share = TimCalendarConfig.init()
    
    public let leftRightMargin: CGFloat = 30
    
    public var itemW: CGFloat {
        get{
            return (UIScreen.main.bounds.size.width - 2 * leftRightMargin ) / 7.0
        }
    }
    
    public var itemH: CGFloat{
        get{
            return itemW
        }
    }
